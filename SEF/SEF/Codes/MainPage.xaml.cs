            ...
            try
            {
                using(HttpClient client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        {
                            UpdateLW();
                            await DisplayAlert($"{filteredEvent}s Not Found", $"None {filteredEvent} happend between {fil.StartDate.ToString("dd-MM-yyyy")} and {fil.EndDate.ToString("dd-MM-yyyy")}", "Ok");
                            return;
                        }
